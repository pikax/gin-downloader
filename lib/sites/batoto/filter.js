...
exports.processFilter = function (filter) {
    ...
    if (search) {
        ...
        fmature = util_1.isNullOrUndefined(mature_1) || mature_1 ? "y" : "n";
        if (rating) {
    ...
    var rating_low = "rating_low=" + (fratingFrom || 0); // 0~5
    var rating_high = "rating_high=" + (fratingTo || 5); // 0~5
    var mature = "mature=" + (fmature || "y"); // n == false
    return { src: config_1.config.mangas_url + "?" + [
            mangaName,
            nameCond,
            authorArtist,
            authorCond,
