...
exports.processFilter = function (filter) {
    ...
    var genreFilter = lodash_1.map(Supported, function (x) { return "genres%5B" + correctName[x].replace(/ /g, "+") + "%5D=" + inOutGenre(x, inGenres, outGenres); }).join("&");
    ...
    return { src: url_1.resolve(config_1.config.site, "/search.php?" + [nameMethod, mangaName,
            type,
            authorMethod, author,
            artistMethod, artist,
            genreFilter,
